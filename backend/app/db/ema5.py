...
def upsert_session(session_id: str, *, session_date: str, mode: str, status: str) -> None:
    now = now_ist().isoformat(timespec="seconds")
    with _DB_LOCK, _connect() as conn:
        conn.execute(
            """
            INSERT INTO ema5_sessions (id, session_date, mode, status, created_at, updated_at)
...
def get_session(session_id: str) -> dict[str, Any] | None:
    with _DB_LOCK, _connect() as conn:
        row = conn.execute("SELECT * FROM ema5_sessions WHERE id = ?", (session_id,)).fetchone()
    return _session_from_row(row) if row else None


def list_sessions(limit: int = 30) -> list[dict[str, Any]]:
    with _DB_LOCK, _connect() as conn:
        rows = conn.execute("SELECT * FROM ema5_sessions ORDER BY session_date DESC LIMIT ?", (limit,)).fetchall()
...
def record_trade_entry(session_id: str, side: str) -> None:
    ...
    with _DB_LOCK, _connect() as conn:
        conn.execute(f"UPDATE ema5_sessions SET {column} = {column} + 1, updated_at = ? WHERE id = ?", (now, session_id))
        conn.commit()
...
def record_trade_result(session_id: str, side: str, *, was_sl: bool) -> None:
    ...
    with _DB_LOCK, _connect() as conn:
        if was_sl:
            conn.execute(f"UPDATE ema5_sessions SET {streak_column} = {streak_column} + 1, updated_at = ? WHERE id = ?", (now, session_id))
        else:
            conn.execute(f"UPDATE ema5_sessions SET {streak_column} = 0, updated_at = ? WHERE id = ?", (now, session_id))
        conn.commit()
...
def set_side_halted(session_id: str, side: str, halted: bool) -> None:
    ...
    with _DB_LOCK, _connect() as conn:
        conn.execute(f"UPDATE ema5_sessions SET {column} = ?, updated_at = ? WHERE id = ?", (1 if halted else 0, now, session_id))
        conn.commit()
...
def record_signal(
    ...
    *,
    ...
    with _DB_LOCK, _connect() as conn:
        cursor = conn.execute(
            """
            INSERT INTO ema5_signals (session_id, side, kind, status, strike, index_level, payload_json, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
...
def update_signal_status(signal_id: int, status: str, *, trade_id: str | None = None) -> None:
    ...
    with _DB_LOCK, _connect() as conn:
        if trade_id is not None:
            conn.execute(
                "UPDATE ema5_signals SET status = ?, trade_id = ?, updated_at = ? WHERE id = ?",
                (status, trade_id, now, signal_id),
            )
        else:
            conn.execute("UPDATE ema5_signals SET status = ?, updated_at = ? WHERE id = ?", (status, now, signal_id))
        conn.commit()


def get_signal(signal_id: int) -> dict[str, Any] | None:
    with _DB_LOCK, _connect() as conn:
        row = conn.execute("SELECT * FROM ema5_signals WHERE id = ?", (signal_id,)).fetchone()
    return _signal_from_row(row) if row else None


def get_signals_for_session(session_id: str) -> list[dict[str, Any]]:
    with _DB_LOCK, _connect() as conn:
        rows = conn.execute("SELECT * FROM ema5_signals WHERE session_id = ? ORDER BY created_at, id", (session_id,)).fetchall()
...
def get_pending_signals(session_id: str) -> list[dict[str, Any]]:
    with _DB_LOCK, _connect() as conn:
        rows = conn.execute(
            "SELECT * FROM ema5_signals WHERE session_id = ? AND status = 'PENDING' ORDER BY created_at, id", (session_id,)
        ).fetchall()
    return [_signal_from_row(row) for row in rows]
...
def insert_trade(
    ...
    *,
    ...
    with _DB_LOCK, _connect() as conn:
        conn.execute(
            """
            INSERT INTO ema5_trades (
                id, session_id, side, strike, security_id, exchange_segment, mode, status,
                entry_signal_id, entry_index_level, initial_sl, target1, target2, target3,
...
def record_entry_fill(trade_id: str, *, entry_premium: float, entry_qty: int) -> None:
    ...
    with _DB_LOCK, _connect() as conn:
        conn.execute(
            "UPDATE ema5_trades SET entry_premium = ?, entry_qty = ?, entry_at = ?, updated_at = ? WHERE id = ?",
...
def update_trade_phase(trade_id: str, phase: str, *, lot3_trail_sl: float | None = None) -> None:
    ...
    with _DB_LOCK, _connect() as conn:
        if lot3_trail_sl is not None:
            conn.execute(
                "UPDATE ema5_trades SET phase = ?, lot3_trail_sl = ?, updated_at = ? WHERE id = ?",
                (phase, lot3_trail_sl, now, trade_id),
            )
        else:
            conn.execute("UPDATE ema5_trades SET phase = ?, updated_at = ? WHERE id = ?", (phase, now, trade_id))
        conn.commit()
...
def close_trade(trade_id: str, *, realized_pnl: float) -> None:
    ...
    with _DB_LOCK, _connect() as conn:
        conn.execute(
            "UPDATE ema5_trades SET status = 'CLOSED', realized_pnl = ?, updated_at = ? WHERE id = ?",
...
def get_trade(trade_id: str) -> dict[str, Any] | None:
    with _DB_LOCK, _connect() as conn:
        row = conn.execute("SELECT * FROM ema5_trades WHERE id = ?", (trade_id,)).fetchone()
    return _trade_from_row(row) if row else None


def get_open_trades(session_id: str) -> list[dict[str, Any]]:
    with _DB_LOCK, _connect() as conn:
        rows = conn.execute("SELECT * FROM ema5_trades WHERE session_id = ? AND status = 'OPEN' ORDER BY created_at", (session_id,)).fetchall()
    return [_trade_from_row(row) for row in rows]


def get_trades_for_session(session_id: str) -> list[dict[str, Any]]:
    with _DB_LOCK, _connect() as conn:
        rows = conn.execute("SELECT * FROM ema5_trades WHERE session_id = ? ORDER BY created_at", (session_id,)).fetchall()
    return [_trade_from_row(row) for row in rows]
...
def insert_trade_legs(trade_id: str, *, lot_count: int, qty_per_lot: int) -> None:
    ...
    with _DB_LOCK, _connect() as conn:
        for lot_number in range(1, lot_count + 1):
            conn.execute(
                """
                INSERT INTO ema5_trade_legs (trade_id, lot_number, qty, status, created_at, updated_at)
                VALUES (?, ?, ?, 'OPEN', ?, ?)
                """,
...
def get_trade_legs(trade_id: str) -> list[dict[str, Any]]:
    with _DB_LOCK, _connect() as conn:
        rows = conn.execute("SELECT * FROM ema5_trade_legs WHERE trade_id = ? ORDER BY lot_number", (trade_id,)).fetchall()
...
def get_legs_for_trades(trade_ids: list[str]) -> dict[str, list[dict[str, Any]]]:
    ...
    with _DB_LOCK, _connect() as conn:
        rows = conn.execute(f"SELECT * FROM ema5_trade_legs WHERE trade_id IN ({placeholders}) ORDER BY trade_id, lot_number", trade_ids).fetchall()
...
def record_event(session_id: str, event_type: str, message: str, payload: dict[str, Any] | None = None) -> None:
    ...
    with _DB_LOCK, _connect() as conn:
        conn.execute(
            "INSERT INTO ema5_events (session_id, event_type, message, payload_json, created_at) VALUES (?, ?, ?, ?, ?)",
...
def get_events_for_session(session_id: str, limit: int = 200) -> list[dict[str, Any]]:
    with _DB_LOCK, _connect() as conn:
        rows = conn.execute(
            "SELECT * FROM ema5_events WHERE session_id = ? ORDER BY created_at DESC, id DESC LIMIT ?", (session_id, limit)
        ).fetchall()
