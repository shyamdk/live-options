"""animesh-scalping orchestration: daily Heikin Ashi bias detection, 1-minute
candle-close MACD-crossover signal detection, live-tick SL/target/trailing
monitoring, and the only code path that ever places an order — always
behind approve_animesh_signal, mirroring ema5.py's manual-approval pattern.

Unlike ema5 (PE@5m, CE@15m — genuinely different candle series), animesh
uses the SAME 1-minute NIFTY candle series for both sides (only the daily
bias differs which side may actually enter), so candles/MACD/EMA-band are
fetched and computed ONCE per poll and shared across both sides' state.

Trade management reuses ema5_engine.evaluate_trade_tick directly (the 3-lot
partial-exit/ratchet-trailing-SL state machine has no EMA5-specific concept
baked into it) — see animesh_engine.py's module docstring.
"""
...
def start_animesh_task() -> asyncio.Task | None:
    settings = get_settings()
    if not settings.animesh_monitor_enabled:
        return None
    return asyncio.create_task(_scheduler_loop())


async def stop_animesh_task(task: asyncio.Task | None) -> None:
...
async def _scheduler_loop() -> None:
    settings = get_settings()
    interval = max(settings.animesh_evaluation_interval_seconds, 1)
    while True:
        try:
...
async def _tick(settings: Settings) -> None:
    now = now_ist()
    in_hours = in_time_window(now.time(), settings.animesh_session_start_time, settings.animesh_session_end_time)
...
async def _start_session(settings: Settings, now: datetime) -> None:
    ...
    try:
        ...
        daily_candles = await fetch_daily_bias_candles(
            dhan, str(settings.dhan_nifty_security_id), settings.animesh_daily_bias_lookback_days
        )
        bias = daily_bias(daily_candles)
        prev_close = daily_candles[-1].close if daily_candles else None
    except Exception:
        bias = None

    db.upsert_session(session_id, session_date=session_id, mode=settings.animesh_mode, status="RUNNING", daily_bias=bias)
    db.record_event(session_id, "SESSION_START", f"animesh session started, mode {settings.animesh_mode}, bias {bias}")
    ...
    _active = {
        ...
        "gapChecked": False,
        "scanBlockedUntilEpoch": None,
        "lastCandlePollAt": 0.0,
        "candles": [],
        "macd": [],
        "signalLine": [],
        "histogram": [],
        "band": {"high": [], "low": [], "median": []},
        "sides": {
            "PE": {"crossoverCandle": None, "lastEntryCandleTime": None},
            "CE": {"crossoverCandle": None, "lastEntryCandleTime": None},
        },
    }
...
async def _end_session() -> None:
    ...
    db.upsert_session(session_id, session_date=session_id, mode=get_settings().animesh_mode, status="COMPLETED")
    db.record_event(session_id, "SESSION_END", "animesh session ended")
    _active = None
...
async def _evaluate_tick(settings: Settings, now: datetime) -> None:
    ...
    await _maybe_refresh_candles(settings, now)
    for side in SIDES:
...
async def _maybe_refresh_candles(settings: Settings, now: datetime) -> None:
    if time.monotonic() - _active["lastCandlePollAt"] < settings.animesh_candle_poll_interval_seconds:
        return
    _active["lastCandlePollAt"] = time.monotonic()
    try:
        dhan = DhanService(settings)
        raw_candles = await fetch_today_candles(
            dhan,
            str(settings.dhan_nifty_security_id),
        ...
        macd_result = compute_macd(
            ...
            fast=settings.animesh_macd_fast,
            slow=settings.animesh_macd_slow,
            signal=settings.animesh_macd_signal,
        )
        _active["macd"] = macd_result["macd"]
        _active["signalLine"] = macd_result["signal"]
        _active["histogram"] = macd_result["histogram"]
        _active["band"] = compute_ema_band(completed, period=settings.animesh_ema_band_period)

        if not _active["gapChecked"] and completed:
            _active["gapChecked"] = True
            prev_close = _active["prevClose"]
            if prev_close is not None:
                gap = abs(completed[0].open - prev_close)
                if gap >= settings.animesh_gap_threshold_points:
                    _active["scanBlockedUntilEpoch"] = completed[0].time + settings.animesh_gap_delay_minutes * 60
    except Exception:
        pass


def _entry_window_ok(now_time: dt_time, settings: Settings) -> bool:
    return in_time_window(now_time, settings.animesh_entry_window_1_start, settings.animesh_entry_window_1_end) or in_time_window(
        now_time, settings.animesh_entry_window_2_start, settings.animesh_entry_window_2_end
    )


async def _maybe_raise_entry_signal(settings: Settings, session_id: str, side: str, now: datetime) -> None:
    if _active["dailyBias"] != side:
        return

    candles = _active["candles"]
    macd = _active["macd"]
    signal_line = _active["signalLine"]
    band = _active["band"]
    min_warmup = settings.animesh_macd_slow + settings.animesh_macd_signal
    ...
    scan = scan_for_crossover_trigger(candles, macd, signal_line, band["high"], band["low"], side)
    side_state["crossoverCandle"] = scan["crossoverCandle"]
    ...
    triggered = scan["triggeredCrossoverCandle"]
    if triggered is None:
        return
    idx = next((i for i, c in enumerate(candles) if c.time == triggered.time), None)
    if idx is None:
        return
    band_low_at_trigger = band["low"][idx]
    band_high_at_trigger = band["high"][idx]
    recent = candles[max(0, idx - 10) : idx]
    initial_sl = compute_initial_sl(
        triggered,
        side,
        band_low_at_trigger=band_low_at_trigger,
        band_high_at_trigger=band_high_at_trigger,
        recent_candles=recent,
        large_candle_multiplier=settings.animesh_large_candle_multiplier,
    )
    entry_level = triggered.high if side == "CE" else triggered.low
    ...
    try:
        ...
        option = (
            await resolve_atm_option(
                ...
                expiry=expiry,
                side=side,
                strike_step=settings.animesh_strike_step,
            )
            if expiry
            else None
        )
...
async def _evaluate_open_trade(settings: Settings, session_id: str, side: str, spot: float, now: datetime) -> None:
    ...
    if now.time() >= _parse_hhmm(settings.animesh_force_exit_time):
...
async def _raise_exit_signal(
    ...
    pending = [s for s in db.get_pending_signals(session_id) if s["kind"] == "EXIT" and s["tradeId"] == trade["id"]]
    if pending:
        return
...
async def _alert_and_maybe_auto_approve(settings: Settings, session_id: str, signal_id: int, phase: str) -> None:
    ...
    if settings.animesh_mode == "PAPER" and settings.animesh_paper_auto_approve:
        await approve_animesh_signal(signal_id)
...
async def _send_alert(settings: Settings, session_id: str, signal_id: int, phase: str) -> None:
    ...
    if now_monotonic - last < settings.animesh_alert_repeat_seconds:
        return
    ...
    if not signal:
        return
    lines = [
        f"⚡ animesh-scalping {signal['side']} {phase.upper()} signal — approval needed",
        f"Kind: {signal['kind']}, Strike: {signal.get('strike')}, Index level: {signal.get('indexLevel')}",
        f"Mode: {settings.animesh_mode}",
    ]
    await TelegramNotifier(settings).send("\n".join(lines))


async def approve_animesh_signal(signal_id: int) -> dict[str, Any]:
    settings = get_settings()
    signal = db.get_signal(signal_id)
    if not signal:
...
async def _approve_entry(settings: Settings, signal: dict[str, Any]) -> dict[str, Any]:
    ...
    quantity = settings.animesh_lots_per_trade * settings.animesh_lot_size
    fill_price, order_status, order_message = await _place_or_simulate(
        ...
        quantity=quantity,
        correlation_id=f"ANIMESH-ENTRY-{signal['id']}",
    )
    ...
    trade_id = f"ANIMESH-{signal['side']}-{payload.get('strike')}-{signal['id']}"
    db.insert_trade(
        trade_id,
    ...
    db.insert_trade_legs(trade_id, lot_count=settings.animesh_lots_per_trade, qty_per_lot=settings.animesh_lot_size)
    ...
    await TelegramNotifier(settings).send(f"✅ animesh {signal['side']} entry filled: {trade_id} at {fill_price:g} x{quantity} ({order_status})")
...
async def _approve_exit(settings: Settings, signal: dict[str, Any]) -> dict[str, Any]:
    ...
    quantity = len(lots) * settings.animesh_lot_size
    if quantity <= 0:
        db.update_signal_status(signal["id"], "REJECTED")
    ...
    fill_price, order_status, order_message = await _place_or_simulate(
        ...
        quantity=quantity,
        correlation_id=f"ANIMESH-EXIT-{signal['id']}",
    )
    if fill_price is None:
        db.update_signal_status(signal["id"], "FAILED")
    ...
    per_lot_pnl = round((fill_price - (trade["entryPremium"] or 0)) * settings.animesh_lot_size, 2)
    ...
    if not remaining_open:
        ...
        if streak >= settings.animesh_max_consecutive_sl_per_side:
    ...
    await TelegramNotifier(settings).send(f"✅ animesh {trade['side']} {action}: {trade_id} at {fill_price:g}, lots {lots}, P&L {total_realized:g}")
...
async def _place_or_simulate(
    ...
    if settings.animesh_mode == "PAPER":
        if ltp is None:
...
async def get_state() -> dict[str, Any]:
    settings = get_settings()
    if _active is None:
        return {"mode": settings.animesh_mode, "status": "NOT_STARTED"}
    ...
    shared_candles = [_candle_dict(c) for c in _active["candles"]]
    band = _active["band"]
    ...
    for side in SIDES:
        ...
        sides[side] = {
            ...
            "macd": _active["macd"],
            "signal": _active["signalLine"],
            "histogram": _active["histogram"],
            "bandHigh": band["high"],
            "bandLow": band["low"],
            "bandMedian": band["median"],
            "crossoverCandle": _candle_dict(side_state["crossoverCandle"]),
            "isBiasActive": _active["dailyBias"] == side,
